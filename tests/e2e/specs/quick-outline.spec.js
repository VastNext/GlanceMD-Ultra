...
test('快速大纲打开后同一事件轮立即 Escape 关闭且不抢回焦点', async ({ page }) => {
  const editor = page.locator('#editor');
  await editor.fill('# 即时关闭\n正文');
  await editor.focus();
  await page.evaluate(() => {
    window.QuickOutline.open();
    document.activeElement.dispatchEvent(new KeyboardEvent('keydown', {
      key: 'Escape', bubbles: true, cancelable: true
    }));
  });
  await expect(page.locator('#quick-outline-overlay')).not.toHaveClass(/open/);
  await expect(editor).toBeFocused();
});
