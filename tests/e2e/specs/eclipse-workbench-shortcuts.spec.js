...
test('文件与保存类 Eclipse 快捷键均分派到真实行为',async({page})=>{await page.keyboard.press('Control+n');await expect(page.locator('.tab')).toHaveCount(1);const e=page.locator('#editor');await e.fill('changed');await e.focus();await page.keyboard.press('Control+s');let log=await page.evaluate(()=>__ipcLog.map(JSON.parse));expect(log.some(m=>m.command==='save_file')).toBeTruthy();await page.keyboard.press('Alt+Shift+s');log=await page.evaluate(()=>__ipcLog.map(JSON.parse));expect(log.filter(m=>m.command==='save_as').length).toBeGreaterThan(0);// 从 __ipcLog 捕获真实 save_file 的 requestId/tabId 回 file_saved（与真实 Rust 回执一致），不再硬编码 requestId
const saveFile=log.filter(m=>m.command==='save_file').pop();expect(saveFile,'Ctrl+S 应分派 save_file').toBeTruthy();expect(saveFile.requestId).toBeGreaterThan(0);await page.evaluate(s=>window.__fromRust('file_saved',{path:'',requestId:s.requestId,tabId:s.tabId}),saveFile);expect(await page.evaluate(()=>TabManager.getActiveTab().dirty)).toBe(false);await page.keyboard.press('Control+w');// 保存已生效：直接关闭即通过；若保存链路失效（tab 仍 dirty）将弹确认且 tab 无法归零
