...
const FIND_FIXTURE = Array.from({ length: 29 }, (_, i) => `line ${i + 1}: is the target`).join('\n');
...
test('问题5：固定 MD 夹具搜索 is 匹配 29 项，Ctrl+K 连续推进绝不死循环', async ({ page }) => {
  const editor = page.locator('#editor');
  const content = FIND_FIXTURE;
...
test('问题14：Preview 跨屏滚动搜索时，查找条保持在固定外壳内绝不随正文滚出视口', async ({ page }) => {
  // 自包含确定性夹具：每章仅 1 个 seo 匹配并以大量填充行隔开（每章约 1250px），
  // 保证第 20 次 Enter 命中的匹配必然在首屏之外，滚动行为可预测且与渲染时序无关
  const content = Array.from({ length: 20 }, (_, i) => [
    `## Chapter ${i + 1} with seo topic`,
    '',
    ...Array.from({ length: 50 }, (_, j) => `Filler paragraph line ${j + 1} for chapter ${i + 1} without the target word.`),
    ''
  ].join('\n')).join('\n');
  ...
  // 连续 Enter 推进到第 12 个匹配（远小于 20 个匹配总数，避免 % 回绕回首个匹配），
  // 此时已深入跨屏滚动区域（scrollTop 约 1.4 万 px）
  for (let i = 0; i < 12; i++) {
    await findInput.press('Enter');
  }
