(function (root) {
  ...
  // 校验 When 表达式语法合法性（空值与 global 视为合法；无解析器时降级放行）
  BindingService.prototype.validateWhen = function (expr) {
    var s = String(expr == null ? '' : expr).trim();
    if (!s || s === 'global') return true;
    try {
      if (root.When && typeof root.When.parse === 'function') { root.When.parse(s); return true; }
      if (typeof root.WhenClause === 'function') { new root.WhenClause(s); return true; }
      return true;
    } catch (e) { return false; }
  };

  BindingService.prototype.active = function (binding) {
    if (binding.platform !== '*' && binding.platform !== this.platform) return false;
    if (!binding.when || binding.when === 'global') return true;
    try {
      return new root.WhenClause(binding.when).evaluate(this.context);
    } catch (e) {
      // 非法 When 表达式：视为 inactive，保证同序列其他有效绑定与后续按键不受影响
      return false;
    }
  };
