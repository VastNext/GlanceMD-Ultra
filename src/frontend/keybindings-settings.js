...
(function () {
  ...
  var FALLBACK_LOCALES = {
    'zh-CN': {
      ...
      'kbRecorder.whenInvalid': '⚠️ 生效条件表达式无效："{when}"，请修正语法后再保存'
    },
    'en': {
      ...
      'kbRecorder.whenInvalid': '⚠️ Invalid when expression: "{when}". Please fix the syntax before saving'
    }
  };
  ...
  // When 归一化：'global' 与空串语义等价（运行时 active() 均按无上下文处理）
  function normalizeWhenKey(w) {
    var s = String(w || '').trim();
    return s === 'global' ? '' : s;
  }

  // 保存前 When 表达式合法性校验：优先复用 BindingService.validateWhen，
  // 未提供时直接用全局 when-clause 解析器；两者皆缺则降级放行（运行时 active() 已有防御）
  function validateWhenExpression(expr) {
    var service = getService();
    if (service && typeof service.validateWhen === 'function') {
      return service.validateWhen(expr);
    }
    var s = String(expr == null ? '' : expr).trim();
    if (!s || s === 'global') return true;
    try {
      if (window.When && typeof window.When.parse === 'function') { window.When.parse(s); return true; }
      if (typeof window.WhenClause === 'function') { new window.WhenClause(s); return true; }
      return true;
    } catch (e) {
      return false;
    }
  ...
  // platform 生效域重叠：任一方为 '*'（全平台）或两者相同，即视为同平台生效
  function platformOverlaps(a, b) {
    return a === '*' || b === '*' || a === b;
  }

  // 保存前冲突检查：直接扫描 service.getBindings() 的全部条目（含同命令多绑定），
  // 仅排除被编辑的目标条目本身；按键序列与上下文相同且 platform 生效域重叠即视为冲突
  function findSaveConflict(commandId, keySequence, when, targetRecord) {
    if (!keySequence) return null;
    var service = getService();
    if (!service) return null;
    var normWhen = normalizeWhenKey(when);
    var targetPlatform = targetRecord ? (targetRecord.platform || '*') : '*';
    var bindings = service.getBindings();
    for (var i = 0; i < bindings.length; i++) {
      var b = bindings[i];
      if (!b.sequence) continue;
      // 跳过被编辑的目标条目本身（同命令且三元组与目标一致的条目，保存后会被替换）
      if (targetRecord &&
          b.commandId === commandId &&
          b.sequence === targetRecord.sequence &&
          normalizeWhenKey(b.when) === normalizeWhenKey(targetRecord.when) &&
          (b.platform || '*') === targetPlatform) continue;
      if (b.sequence === keySequence &&
          normalizeWhenKey(b.when) === normWhen &&
          platformOverlaps(b.platform || '*', targetPlatform)) {
        var meta = resolveCommandMeta(b.commandId);
        return { commandId: b.commandId, title: meta.label, key: b.sequence, when: b.when };
      }
    }
    return null;
  }

  // 保存绑定：保存前校验 When 合法性与按键冲突（拒绝保存而非仅提示）；
  // 仅替换被编辑的目标绑定（previousSequence 或当前第一条），保留同命令其余绑定，
  // 并继承目标的 passthrough 与 platform，避免破坏剪切复制等原生放行与平台专属生效域
  function saveBinding(commandId, keySequence, when, previousSequence) {
    if (!commandId) return { ok: false, reason: 'invalid-command' };

    // 校验 1：When 表达式合法性，非法拒绝落盘
    if (!validateWhenExpression(when)) {
      return { ok: false, reason: 'invalid-when', when: String(when == null ? '' : when) };
    }

    var service = getService();
    // 该命令当前生效的全部条目（service 的 override 语义是命令级全量替换）
    var current = service
      ? service.getBindings().filter(function (b) { return b.commandId === commandId; })
      : [];
    // 定位被编辑的目标条目：优先匹配 previousSequence，未命中/未指定时视为第一条
    var targetIndex = -1;
    if (previousSequence) {
      for (var i = 0; i < current.length; i++) {
        if (current[i].sequence === previousSequence) { targetIndex = i; break; }
      }
    }
    if (targetIndex < 0) targetIndex = current.length > 0 ? 0 : -1;
    var targetRecord = targetIndex >= 0 ? current[targetIndex] : null;

    // 编辑时保留目标的 passthrough（剪切/复制等原生放行）与 platform 生效域
    var newRecord = {
      commandId: commandId,
      sequence: keySequence || '',
      when: when || '',
      platform: targetRecord ? (targetRecord.platform || '*') : '*',
      source: 'user',
      removed: !keySequence,
      passthrough: targetRecord ? Boolean(targetRecord.passthrough) : false
    };

    // 校验 2：与任意绑定条目存在同上下文、平台重叠的按键冲突，拒绝保存
    var conflict = findSaveConflict(commandId, keySequence, when, targetRecord);
    if (conflict) {
      return { ok: false, reason: 'conflict', conflict: conflict };
    }

    // 目标之外的条目原样保留进 override，避免命令级替换吞掉同命令其他绑定
    var siblings = [];
    current.forEach(function (b, index) {
      ...
      siblings.push({
        ...
        source: 'user',
        removed: false,
        passthrough: Boolean(b.passthrough)
      });
    });

    // 与新记录重复的兄弟条目（相同按键+上下文+平台）以新记录为准；
    // 新记录排在首位，保证 UI（computeItems 显示 overrides[0]）呈现用户刚编辑的绑定
    var merged = siblings.filter(function (b) {
      return !(b.sequence === newRecord.sequence &&
        normalizeWhenKey(b.when) === normalizeWhenKey(newRecord.when) &&
        b.platform === newRecord.platform);
    });
    merged.unshift(newRecord);

    var overrides = Object.assign({}, state.overrides);
    overrides[commandId] = merged;
    saveOverrides(overrides);
    render();
    return { ok: true };
  }
  ...
  // 弹层统一保存入口：校验被拒时保持弹层打开并展示错误，允许用户修正后重试
  function commitRecorderSave() {
    var whenInput = document.getElementById('kb-recorder-when-input');
    var result = saveBinding(
      state.recorder.commandId,
      state.recorder.recordedKey,
      whenInput ? whenInput.value : 'global',
      state.recorder.originalKey
    );
    if (result && result.ok === false) {
      // 保存被拒：保持弹层打开；重置录制状态，避免残留的 chord 首段
      // 把用户下一次按键误拼为组合键。先重算录制区显示，再叠加错误提示
      state.recorder.firstStroke = null;
      state.recorder.isChordWaiting = false;
      updateRecorderDisplay();
      showRecorderError(result);
      return;
    }
    closeRecorder();
  }
  ...
  // 在弹层冲突提示区展示保存被拒原因
  function showRecorderError(result) {
    var overlay = document.getElementById('kb-recorder-overlay');
    if (!overlay) return;
    var el = overlay.querySelector('#kb-recorder-conflict-warning');
    if (!el) return;
    var text = '';
    if (result.reason === 'invalid-when') {
      text = t('kbRecorder.whenInvalid', { when: result.when || '' });
    } else if (result.reason === 'conflict' && result.conflict) {
      text = t('kbRecorder.conflictWarning', {
        name: result.conflict.title || result.conflict.commandId,
        id: result.conflict.commandId
      });
    }
    if (!text) return;
    el.textContent = text;
  ...
  function openRecorder(commandId) {
    ...
    state.recorder.originalKey = item.key || '';
