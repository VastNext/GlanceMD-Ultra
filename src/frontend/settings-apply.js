...
// - apply(effective, fromDocument) 把 editor.fontSize / editor.tabSize /
//   editor.wordWrap / editor.lineNumbers / appearance.sidebarFontSize 写入对应载体：
...
// - 快捷键：有效设置（或全局文档）的 keybindings 段在 fromDocument 时传给
//   window.Keybindings.loadFromSettings 装载（keybindings 仅全局设置，项目级被
//   Rust effective 忽略；写入/迁移由 keybindings.js 的设置桥接负责）；
// - get() 返回最近一次 effective（未收到事件时返回与 Rust schema v2 一致的
...
(function () {
  ...
  // 内置默认值：与 Rust settings.rs schema v2 的 Default 一致。
  var DEFAULTS = {
  ...
  // 应用一份有效设置（workspace:settings-effective 的 d.settings）。fromDocument
  // 为真时表示来源是设置文档回执（区别于 init 的内置默认值），此时把 keybindings
  // 段传给 Keybindings 门面装载。
  function apply(effective, fromDocument) {
    var s = effective || {};
    var ed = s.editor || {};
    var ap = s.appearance || {};
    ...
    // 快捷键：仅真实设置文档的 keybindings 段驱动 Keybindings 门面（幂等装载，
    // 项目级 keybindings 已被 Rust effective 忽略，此处恒为全局值）。
    if (
      fromDocument &&
      s.keybindings &&
      window.Keybindings &&
      typeof window.Keybindings.loadFromSettings === 'function'
    ) {
      window.Keybindings.loadFromSettings(s.keybindings);
    }
