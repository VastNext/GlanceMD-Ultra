  ...
  function toggleVimMode(force) {
    ...
    if (next) {
      ...
      if (window.VimUI && typeof window.VimUI.showToast === 'function') {
        var enterMsg = (window.I18n && typeof window.I18n.t === 'function' && window.I18n.t('vim.toast.enter')) || '已进入 Vim 模式（普通模式）';
        window.VimUI.showToast(enterMsg, 1500);
      }
    } else {
      vimEnabled = false;
      if (window.VimUI && typeof window.VimUI.showToast === 'function') {
        var exitMsg = (window.I18n && typeof window.I18n.t === 'function' && window.I18n.t('vim.toast.exit')) || '已退出 Vim 模式';
        window.VimUI.showToast(exitMsg, 1500);
      }
