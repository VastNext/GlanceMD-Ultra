...
| 指标 | 实测 | 门禁 | 结论 |
|---|---|---|---|
| 根目录 `list_dir` | 0.553ms（5 条目） | 单目录 < 50ms | 通过 |
| 全树 60 目录逐层 `list_dir` 累计 | 72.465ms（平均 1.208ms/目录） | — | 前端全量展开一次的累计成本仍远低于 UI 卡顿阈值 |
| 单目录最大 `list_dir` | 2.812ms（branch-6/level-2/level-3/level-4/level-5/level-6，155 条目） | 单目录 < 50ms | **通过（≈1/18）** |
| 搜索首结果（高频查询 `needle-`） | **1.549ms** | 首结果 < 1s | **通过** |
| 搜索全量（高频查询，截断于 10000 条上限） | 2.935s（扫描 6219 文件） | 全量 < 5s | **通过** |
| 搜索全量（唯一锚点 `needle-04217`，扫完全部 10000 文件） | **4.864s**（命中 2，扫描 10000 文件） | 全量 < 5s | **通过（余量约 136ms）** |
| 取消粒度 | 每文件检查点（实测单文件处理毫秒级） | 线程 500ms 内退出 | 通过（预期） |

- 正确性联动：10k 测试同时断言全树可见文件恰为 10000（过滤语义正确性）、全量搜索 `files_scanned == 10000`。
- 附注：全量 4.864s 主要是 10k 次文件打开的 I/O 成本；当前只剩约 136ms 门禁余量，若目标硬件更慢，阶段 4 可评估按目录并行扫描或索引缓存；当前实测满足门禁，暂不引入额外依赖。
