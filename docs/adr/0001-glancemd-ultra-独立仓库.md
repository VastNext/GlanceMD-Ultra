...
2. **轻量约束重新表述**。原"GlanceMD 的单文件体验不能因 Ultra 退化"约束对本仓库不再适用；Ultra 自身的轻量目标为：二进制 2–8 MB 以内（2026-09-09 由 2–5 MB 放宽以容纳工作区化能力，减重计划见 backlog FEAT-004）、无 Electron/Monaco、保持无外部运行时依赖（安装包自包含）。
